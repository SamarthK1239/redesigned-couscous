...
cur.execute(
    "CREATE TABLE IF NOT EXISTS subjects(Course VARCHAR(255), Code VARCHAR(10), Credits INT, GA INT, GHW INT, GH INT, GN INT, GS INT, Rating VARCHAR(15), ABSRating INT);")
...
def insert(filename, types, lst1):
    ...
    for i in range(len(df_arts)):
        ...
        if r == 0:
            cur.execute(
                "insert into subjects (Course, Code, Credits, GA, GHW, GH, GN, GS, Rating, ABSRating) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NULL, NULL);",
                (df_arts.loc[i].at['Course'], df_arts.loc[i].at['Code'], AsIs(df_arts.loc[i].at['Credits']), lst1[0], lst1[1],
                 lst1[2], lst1[3], lst1[4]))
>>>>>>> 97e0f59c3b40beb8098ff44836c929ac09ea9f57
            conn.commit()
        else:
...
lst = [["arts_gened.csv", "GA", [1, 0, 0, 0, 0]], ["humanities.csv", "GH", [0, 1, 0, 0, 0]],
       ["health_wellness.csv", "GHW", [0, 0, 1, 0, 0]], ["nat_sci.csv", "GN", [0, 0, 0, 1, 0]],
       ["social_behavior.csv", "GS", [0, 0, 0, 0, 1]]]
for i in lst:
    insert(i[0], i[1], i[2])
>>>>>>> 97e0f59c3b40beb8098ff44836c929ac09ea9f57
